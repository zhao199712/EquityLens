...
public sealed class FinalizeReanalysisNodeHandler(IAnswerQualityValidator? qualityValidator = null) : IAgentNodeHandler
{
    ...
    public Task ExecuteAsync(AgentNodeExecutionContext context, CancellationToken cancellationToken = default)
    {
        ...
        var allowUncitedInsufficiency = board[AgentBlackboardKeys.RouteDecision]?.GetValue<string>() == "InsufficientEvidence"
            || decision.RecommendedNextAction == "AcceptAnswer" && LlmEvidenceRemediationAgent.IsAbstention(revision.RevisedAnswer);
        var quality = (qualityValidator ?? new AnswerQualityValidator()).Validate(revision.RevisedAnswer, mode, claims, required, supportedIds, allowedIndexes, allowUncitedInsufficiency);
