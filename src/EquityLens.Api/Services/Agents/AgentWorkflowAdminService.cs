...
public interface IAgentWorkflowAdminService
{
    ...
    Task<IReadOnlyDictionary<string, bool>> GetApprovalPoliciesAsync(IEnumerable<string> nodeTypes, CancellationToken ct = default);
}
...
public sealed class AgentWorkflowAdminService : IAgentWorkflowAdminService
{
    ...
    public async Task<AgentNodeAdminResponse?> UpdateNodeAsync(string type, UpdateAgentNodeSettingRequest request, CancellationToken ct = default)
    {
        ...
        setting.IsEnabled = request.IsEnabled; setting.DisplayName = request.DisplayName?.Trim(); setting.Description = request.Description?.Trim(); setting.MetadataJson = request.Metadata?.ValueKind == JsonValueKind.Null ? null : request.Metadata?.GetRawText(); setting.TimeoutSeconds = request.TimeoutSeconds; setting.MaxRetryCount = request.MaxRetryCount; setting.RequiresHumanApprovalOverride = request.RequiresHumanApprovalOverride; setting.UpdatedAtUtc = DateTime.UtcNow;
    ...
    public async Task<IReadOnlyDictionary<string, bool>> GetApprovalPoliciesAsync(IEnumerable<string> nodeTypes, CancellationToken ct = default)
    {
        var types = nodeTypes.Distinct().ToArray();
        var settings = await db.AgentNodeSettings.Where(x => types.Contains(x.NodeType)).ToDictionaryAsync(x => x.NodeType, ct);
        return types.ToDictionary(
            x => x,
            x => settings.GetValueOrDefault(x)?.RequiresHumanApprovalOverride ?? catalog.GetNode(x).Contract.RequiresHumanInput);
    }
    ...
    private static AgentNodeAdminResponse Map(AgentNodeCatalogEntry x, AgentNodeSetting? s)
    {
        var requiresApproval = s?.RequiresHumanApprovalOverride ?? x.Contract.RequiresHumanInput;
        return new(x.NodeType, s?.DisplayName ?? x.DisplayName, s?.Description ?? x.Description, x.Stage, x.SideEffectLevel, s?.IsEnabled ?? true, s?.TimeoutSeconds ?? x.DefaultPolicy.TimeoutSeconds, s?.MaxRetryCount ?? x.DefaultPolicy.MaxRetryCount, requiresApproval, s?.RequiresHumanApprovalOverride, s?.RequiresHumanApprovalOverride is null ? "Catalog" : "AdminOverride", ParseMetadata(s?.MetadataJson), x.Contract, x.RequiredBlackboardKeys, x.ProducedBlackboardKeys, x.AllowedNextNodeTypes);
    }
