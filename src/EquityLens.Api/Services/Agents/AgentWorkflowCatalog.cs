...
public sealed class AgentWorkflowCatalog : IAgentWorkflowCatalog
{
    ...
    [
        ...
        ,N(EvidenceRemediationNodeTypes.AssessSupport, "評估 Claim 支持", "評估補充證據對各 claim 的語義支持程度。", "Analysis", "ExternalLlmRead", [AgentBlackboardKeys.ExtractedClaims, AgentBlackboardKeys.RetrievedEvidence], [AgentBlackboardKeys.ClaimSupportAssessments], [EvidenceRemediationNodeTypes.ValidateMappings])
        ,N(EvidenceRemediationNodeTypes.ValidateMappings, "驗證證據映射", "Deterministic 驗證 citation 索引與數值映射。", "Validation", "ReadOnly", [AgentBlackboardKeys.ExtractedClaims, AgentBlackboardKeys.RetrievedEvidence, AgentBlackboardKeys.ClaimSupportAssessments], [AgentBlackboardKeys.EvidenceValidationResults], [EvidenceRemediationNodeTypes.BuildPacket])
        ,N(EvidenceRemediationNodeTypes.Route, "決定補證據路由", "依驗證結果決定完成或進入下一輪補檢索。", "Routing", "ReadOnly", [AgentBlackboardKeys.EvidenceValidationResults], [AgentBlackboardKeys.RouteDecision, AgentBlackboardKeys.UnresolvedClaims], [EvidenceRemediationNodeTypes.PlanRetrieval, EvidenceRemediationNodeTypes.BuildPacket])
