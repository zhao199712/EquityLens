...
export interface NodeAdmin { nodeType:string; displayName:string; description:string; stage:string; sideEffectLevel:string; isEnabled:boolean; timeoutSeconds:number; maxRetryCount:number; requiresHumanApproval:boolean; requiresHumanApprovalOverride:boolean|null; approvalPolicySource:string; metadata:Record<string, unknown>|null; contract:NodeContract; requiredBlackboardKeys:string[]; producedBlackboardKeys:string[]; allowedNextNodeTypes:string[] }
export interface UpdateNodeSetting { isEnabled:boolean; displayName:string; description:string; timeoutSeconds:number; maxRetryCount:number; requiresHumanApprovalOverride:boolean|null; metadata:Record<string, unknown>|null }
