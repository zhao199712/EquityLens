...
<template>
  <main class="prestige-section prestige-fade admin-view">
    <div class="prestige-section-head">
      <div>
        <p class="prestige-label view-eyebrow">{{ t('admin.riskModels.titleEn') }}</p>
        <h1 class="prestige-section-title">{{ t('admin.riskModels.title') }}</h1>
      </div>
      <button class="prestige-btn prestige-btn-solid">
    ...
    <div class="prestige-empty">
      <NEmpty :description="t('admin.riskModels.emptyTitle')">
        <template #extra>
          <NButton class="empty-btn">{{ t('admin.riskModels.learnMore') }}</NButton>
        </template>
      </NEmpty>
    </div>
  </main>
...
.empty-btn :deep(.n-button__border) {
  border-color: var(--gold-border) !important;
}

.empty-btn :deep(.n-button__state-border) {
