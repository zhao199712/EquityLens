// Mock data for visualization blocks that backend does not provide yet.
// These are explicitly labeled as mock in the UI.
...
export const varResults: VaRResult[] = [
...
export const esResults: ESResult[] = [
...
export const monteCarloPercentiles = {
  p1: Array.from({ length: 253 }, (_, i) => {
    return -0.15 * Math.sqrt(i / 252) - 0.02 * (i / 252)
  }),
  p5: Array.from({ length: 253 }, (_, i) => {
    return -0.10 * Math.sqrt(i / 252) - 0.01 * (i / 252)
  }),
  p50: Array.from({ length: 253 }, (_, i) => {
    return 0.05 * (i / 252)
  }),
  p95: Array.from({ length: 253 }, (_, i) => {
    return 0.12 * Math.sqrt(i / 252) + 0.03 * (i / 252)
  }),
  p99: Array.from({ length: 253 }, (_, i) => {
    return 0.18 * Math.sqrt(i / 252) + 0.04 * (i / 252)
  }),
}
...
export function severityColor(s: StressScenario['severity']) {
  const map = { low: '#666666', medium: '#FF6B00', high: '#8B1A2B', extreme: '#8B1A2B' }
  return map[s]
}

export function severityLabel(s: StressScenario['severity']) {
