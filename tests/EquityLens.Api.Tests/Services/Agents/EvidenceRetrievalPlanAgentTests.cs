...
public sealed class EvidenceRetrievalPlanAgentTests
{
    ...
    {
        var chat = new FakeChat(["{\"searches\":[{\"documentType\":\"AnnualReport\",\"sourceRole\":\"Primary\",\"query\":\"海外設廠成本\",\"topK\":6,\"reason\":\"補足成本證據\"}]}"]);
    ...
    {
        var chat = new FakeChat(["{\"searches\":[]}", "{\"searches\":[{\"documentType\":null,\"sourceRole\":\"Primary\",\"query\":\"供應鏈風險\",\"topK\":5,\"reason\":\"repair\"}]}"]);
        var result = await new EvidenceRetrievalPlanAgent(chat, new FakeFallback()).PlanAsync(Input());
    ...
    {
        var chat = new FakeChat(["not-json", "{\"searches\":[{\"documentType\":null,\"sourceRole\":\"Primary\",\"query\":\"already used\",\"topK\":20,\"reason\":\"invalid\"}]}"]);
