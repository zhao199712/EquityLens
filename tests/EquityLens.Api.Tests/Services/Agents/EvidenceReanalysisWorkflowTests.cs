...
public sealed class EvidenceReanalysisWorkflowTests
{
    ...
    public async Task SevenHandlers_ProduceFinalOutputWithoutLoop()
    {
        ...
        var output = JsonSerializer.Deserialize<EvidenceReanalysisOutput>(run.OutputJson!, AgentNodeJson.SerializerOptions)!; Assert.Equal("重新分析 [1]", output.FinalAnswer); Assert.False(output.RequiresMoreEvidence); Assert.Equal("AcceptAnswer", output.RecommendedNextAction); Assert.Single(output.Citations); Assert.Contains(events, x => x.Type == AgentEventTypes.SupervisorDecision); Assert.Contains(db.AgentToolCalls.Local, x => x.ToolName == "investmentReanalysisLLM");
    }
