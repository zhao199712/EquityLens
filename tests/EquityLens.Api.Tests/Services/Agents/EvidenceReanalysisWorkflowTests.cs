...
public sealed class EvidenceReanalysisWorkflowTests
{
    ...
    public async Task SevenHandlers_ProduceFinalOutputWithoutLoop()
    {
        ...
        await new CritiqueReanalysisNodeHandler(new DeterministicCriticReviewAgent(), [new EvidenceReanalysisPolicyEvaluator()]).ExecuteAsync(new(db, run, run.Nodes.ElementAt(4), writer)); var correctingRevision = new CorrectingRevisionAgent(); await new ReviseReanalysisNodeHandler(correctingRevision, new FinalEntailmentCritic(), [new EvidenceReanalysisPolicyEvaluator()]).ExecuteAsync(new(db, run, run.Nodes.ElementAt(5), writer)); await new FinalizeReanalysisNodeHandler().ExecuteAsync(new(db, run, run.Nodes.ElementAt(6), writer));
        var output = JsonSerializer.Deserialize<EvidenceReanalysisOutput>(run.OutputJson!, AgentNodeJson.SerializerOptions)!; Assert.Equal("已修正年份 [1]", output.FinalAnswer); Assert.False(output.RequiresMoreEvidence); Assert.Equal("AcceptAnswer", output.RecommendedNextAction); Assert.Single(output.Citations); Assert.Equal("Complete", output.AnswerQualityStatus); Assert.Equal(2, correctingRevision.CallCount); Assert.NotNull(AgentNodeJson.ParseBlackboard(run.BlackboardJson)[AgentBlackboardKeys.AnswerQualityValidation]); Assert.Contains(events, x => x.Type == AgentEventTypes.SupervisorDecision); Assert.Contains(db.AgentToolCalls.Local, x => x.ToolName == "investmentReanalysisLLM"); Assert.Contains(db.AgentToolCalls.Local, x => x.ToolName == "finalEntailmentCriticLLM"); Assert.Contains(db.AgentToolCalls.Local, x => x.ToolName == "finalCorrectionRevisionAgent"); Assert.Contains(db.AgentToolCalls.Local, x => x.ToolName == "finalEntailmentRecheckLLM");
    }
    ...
    private sealed class CorrectingRevisionAgent : IDraftRevisionAgent
    {
        ...
        public Task<DraftRevisionResult> ReviseAsync(DraftRevisionInput input, CancellationToken cancellationToken = default) { CallCount++; return Task.FromResult(new DraftRevisionResult(CallCount == 1 ? "錯誤年份 [1]" : "已修正年份 [1]", "修訂", input.AppliedRecommendation)); }
    }
    private sealed class FinalEntailmentCritic : ICriticReviewAgent
    {
        public Task<CriticReviewResult> CritiqueAsync(CriticReviewInput input, CancellationToken cancellationToken = default) => Task.FromResult(input.Answer!.Contains("錯誤年份", StringComparison.Ordinal)
            ? new CriticReviewResult("年份未受支持", "Medium", [new("Medium", "UnsupportedClaim", "年份不受引用支持", [1], "移除錯誤年份")], "已修正年份 [1]")
            : new CriticReviewResult("通過", "None", [], null));
    }
